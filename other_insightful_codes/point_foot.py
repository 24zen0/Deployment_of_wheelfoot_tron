...
class PointFoot:
    ...
    def _compose_privileged_obs_buf_no_height_measure(self):
        self.privileged_obs_buf = torch.cat((self.base_ang_vel * self.obs_scales.ang_vel,
                                             ...
                                             # adding some extra info for the critic
                                             self.base_lin_vel * self.obs_scales.lin_vel,#3
                                             self.base_ang_vel * self.obs_scales.ang_vel,#3
                                             self.contact_filt, #2
                                             self.first_contact, #2
                                             self.feet_air_time, #2
                                             self.current_max_feet_height, #2
                                             self.last_max_feet_height, #2
                                             self.last_feet_air_time, #2
                                             self.feet_height,#2
                                            #  self.feet_state, #26
                                             ), dim=-1)
    ...
    def _resample_commands(self, env_ids):
        ...
        self.commands[env_ids, 1] = torch_rand_float(self.command_ranges["lin_vel_y"][0]*(abs(self.command_ranges["lin_vel_x"][0])>0.1),
                                                     self.command_ranges["lin_vel_y"][1]*(abs(self.command_ranges["lin_vel_x"][1])>0.1), (len(env_ids), 1),
                                                     device=self.device).squeeze(1)
        if self.cfg.commands.heading_command:
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        # robots that walked far enough progress to harder terainscurriculum
    ...
    def update_command_curriculum(self, env_ids):
        ...
        if self.command_ranges["lin_vel_x"][0] < 0:
            scale = 0.65
        else:
            scale = 0.72
        if torch.mean(self.episode_sums["tracking_lin_vel"][env_ids]) / self.max_episode_length > scale * \
                self.reward_scales["tracking_lin_vel"] * scale:
    ...
    def _init_buffers(self):
        ...
        # adding base linear vel and angular vel 
    ...
    def _reward_feet_air_time_sync(self):
        """Reward synchronization of feet air time between consecutive steps"""
        # When a foot contacts the ground (first_contact), we compare its air time
        # with the last air time of the other foot
        left_foot_contact = self.first_contact[:, 0]
        right_foot_contact = self.first_contact[:, 1]
        
        # When left foot contacts, compare its air time with right foot's last air time
        left_sync_reward = torch.zeros_like(self.rew_buf)
        valid_left_contact = left_foot_contact & (self.feet_air_time[:, 0] > self.cfg.rewards.min_feet_air_time)
        left_sync_reward[valid_left_contact] = torch.exp(-torch.abs(
            self.feet_air_time[valid_left_contact, 0] - 
            self.last_feet_air_time[valid_left_contact, 1])*10) 
        
        # When right foot contacts, compare its air time with left foot's last air time
        right_sync_reward = torch.zeros_like(self.rew_buf)
        valid_right_contact = right_foot_contact & (self.feet_air_time[:, 1] > self.cfg.rewards.min_feet_air_time)
        right_sync_reward[valid_right_contact] = torch.exp(-torch.abs(
            self.feet_air_time[valid_right_contact, 1] - 
            self.last_feet_air_time[valid_right_contact, 0])*10) 
        
        return left_sync_reward + right_sync_reward
    def _reward_max_feet_height_sync(self):
        """Reward synchronization of maximum feet height between consecutive steps.
           Computed when feet are in swing phase and air_time exceeds 0.62 * min_air_time.
        """
        reward = torch.zeros_like(self.rew_buf)
        # Only give reward if feet height exceeded target height
        left_foot_valid = (self.last_max_feet_height[:, 0] > self.cfg.rewards.target_feet_height) & self.first_contact[:,0]
        right_foot_valid = (self.last_max_feet_height[:, 1] > self.cfg.rewards.target_feet_height) & self.first_contact[:,1]
        
        reward[left_foot_valid] = torch.exp(-torch.abs(
            self.last_max_feet_height[left_foot_valid, 0] - 
            self.last_max_feet_height[left_foot_valid, 1])*10)
            
        reward[right_foot_valid] = torch.exp(-torch.abs(
            self.last_max_feet_height[right_foot_valid, 1] - 
            self.last_max_feet_height[right_foot_valid, 0])*10)
        return reward
    def _reward_feet_air_time(self):
        ...
        # rew_airTime = torch.min(torch.sum(self.feet_air_time*self.first_contact,dim=1),torch.ones_like(self.rew_buf)*self.cfg.rewards.max_feet_air_time)
        return rew_airTime
    def _reward_feet_height(self):
        """Penalizes swing legs if their clearance from the ground is too low."""
        swing_mask = ~self.contact_filt
        rew_pos = (self.current_max_feet_height > self.cfg.rewards.target_feet_height) 
        rew_pos = torch.sum(rew_pos * swing_mask,dim=1)
        rew_pos[rew_pos>1]=0
        return rew_pos
    def _reward_feet_contact_number(self):
        contact_number = torch.sum(self.contact_filt.float(),dim=1)
        reward = torch.zeros_like(contact_number)
        reward[contact_number==2] = 0.3
        reward[contact_number==1] = 1
        reward[contact_number==0] = -1
        return reward
    def _reward_survival(self):
        return (~self.reset_buf).float()
    def _reward_tracking_lin_vel(self):
        lin_vel_error = torch.sum(torch.square(self.commands[:, :2] - self.base_lin_vel[:, :2]), dim=1)
        return torch.exp(-lin_vel_error / (self.cfg.rewards.tracking_sigma ))#+ torch.sum(direction_match.float(),dim=1)*0.1 # 增加灵敏度
    def _reward_tracking_ang_vel(self):
        ang_vel_error = torch.square(
            (self.commands[:, 2] - self.base_ang_vel[:, 2]))
        return torch.exp(-ang_vel_error/self.cfg.rewards.tracking_sigma)#+direction_match.float()*0.1
    def _reward_vel_mismatch_exp(self):
        # 抑制垂直方向（Z轴）的线速度波动
        lin_mismatch = torch.exp(-torch.square(self.base_lin_vel[:, 2]) * 10)  # Z轴速度平方的指数衰减
        # 抑制非转向轴（X/Y轴）的角速度旋转（俯仰/翻滚）
        ang_mismatch = torch.exp(-torch.norm(self.base_ang_vel[:, :2], dim=1) * 5.)  # X,Y轴角速度的L2范数衰减
        # 综合两种速度误差的加权平均
        c_update = (lin_mismatch + ang_mismatch) / 2.
        return c_update
    def _reward_track_vel_hard(self):
        lin_vel_error = torch.sum(torch.square(self.commands[:, :2] - self.base_lin_vel[:, :2]), dim=1)
        return -lin_vel_error#+ torch.sum(direction_match.float(),dim=1)*0.1 # 增加灵敏度
    def _reward_feet_distance(self):
        foot_dist = torch.norm(self.feet_state[:, 0, :2] - self.feet_state[:, 1, :2], dim=1)
        ...
        return (torch.exp(-d_min.abs()*200)*0.8+torch.exp(-d_max.abs()*100)*0.2)
    def _reward_knee_distance(self):
        foot_pos = self.rigid_body_states[:, self.knee_indices, :2]
        # print(self.knee_indices)
    ...
    def _reward_feet_contact_forces(self):
        return torch.sum((torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1) - self.cfg.rewards.max_feet_contact_forces).clip(0.,350),dim=1)
 
    def _reward_low_speed(self):
        abs_spd = self.base_lin_vel[:,0].abs()
        command_spd = self.commands[:,0].abs()

        spd_slow = abs_spd < command_spd*0.5
        spd_high = abs_spd > command_spd*1.2
        spd_desired = ~spd_slow & ~spd_high
        sign_mismatch = torch.sign(self.commands[:,0]) != torch.sign(self.base_lin_vel[:,0])
        reward = torch.zeros_like(abs_spd)
        reward[spd_slow] = -1.
        reward[spd_high] = 0.
        reward[spd_desired ] = 2.
        reward[sign_mismatch] = -2.
        return reward*(command_spd>0.1)
    # def _reward_stand_still(self):
    #     same_sign = torch.sign(self.last_base_lin_vel[:,0]) == torch.sign(self.base_lin_vel[:,0])
    #     return -same_sign.float()*(torch.norm(self.commands[:,:2],dim=1)<0.1)
    
    def _reward_dof_vel(self):
        return torch.sum(torch.square(self.dof_vel), dim=1)
    def _reward_knee_pos_limit(self):
        # 将2,5的膝盖关节限制在绝对值1以内
        reward = torch.zeros_like(self.rew_buf)
        # Penalize when knee joints (indices 2 and 5) exceed 0.8,to 1
        restrict_pos = 0.8
        scale = 1/(1-restrict_pos)
        reward = torch.min((scale*(restrict_pos-self.dof_pos[:,2].abs())).clamp(min=-1,max=0),(scale*(restrict_pos-self.dof_pos[:,5].abs())).clamp(min=-1,max=0))
        return reward
    def _reward_feet_contact_forces(self):
        """
    ...
    def _reward_dof_action_diff(self):
        """Penalize the difference between dof positions and actions"""
        # Calculate the difference between dof_pos and actions (scaled by action_scale)
        action_scaled = self.actions * self.cfg.control.action_scale
        dof_pos_normalized = (self.dof_pos - self.default_dof_pos) / self.cfg.control.action_scale
        
        # Compute the squared error between dof_pos and actions
        error = torch.sum(torch.abs(dof_pos_normalized - action_scaled), dim=1)
        
        # Return the negative error (to penalize large differences)
        return error
    def _reward_base_height(self):
        # Penalize base height away from target
        base_height = torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights, dim=1)
        # adding base_height rewarding
        return torch.exp(-torch.square(base_height - self.cfg.rewards.base_height_target)*10)
