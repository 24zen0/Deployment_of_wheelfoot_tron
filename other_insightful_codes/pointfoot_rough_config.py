...
class PointFootRoughCfg(BaseConfig):
    class env:
        ...
        num_privileged_obs = 148+20   # if not None a priviledge_obs_buf will be returned by step() (critic obs for assymetric training). None is returned otherwise
        num_actions = 6
    ...
    class rewards:
        class scales:                    
            ...
            #gait
            feet_air_time = 20.0
            feet_air_time_sync = 2.0  # Scale for the new synchronization reward
            feet_contact_number = .3
            max_feet_height_sync = 1.0
            #motion scales
            tracking_lin_vel = 1.5
            tracking_ang_vel = .8
            # vel_mismatch_exp = 0.5 #惩罚z轴线速度，还有惩罚xy轴的角速度
            track_vel_hard = .3 #奖励lin和vel,并且惩罚绝对的差值
            low_speed = 0.1 #奖励方向相同低速，惩罚方向相反速度
